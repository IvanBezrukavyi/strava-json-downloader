...
from utils import (
    to_epoch_seconds_from_date_str,
    now_stamp,
    ensure_data_dir,
    now_epoch_utc,
...
def parse_args() -> argparse.Namespace:
    ...
    f.add_argument(
        ...
        dest="only_runs",
        action="store_true",
        help="Filter to type == Run (default)",
    )
    f.add_argument(
        "--all-types",
        dest="only_runs",
        action="store_false",
        help="Include all activity types",
    )
    ...
    f.add_argument(
        "--append",
        action="store_true",
        help="Append to --out by merging & deduping by activity id",
    )
...
def summarize(activities: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    dist_m = sum((a.get("distance") or 0) for a in activities)
    moving_s = sum((a.get("moving_time") or 0) for a in activities)
    dist_km = dist_m / 1000.0
    hours = moving_s / 3600.0
    pace_sec_per_km = (moving_s / dist_km) if dist_km > 0 else float("nan")
    ...
    for a in activities:
        ...
        if isinstance(sd, str) and (latest_dt is None or sd > latest_dt):
            latest_dt = sd

    return {
        "total_activities": total,
        "total_distance_km": round(dist_km, 2),
        "total_moving_hours": round(hours, 2),
        "avg_pace_min_per_km": fmt_min_per_km(pace_sec_per_km),
        "longest_run_km": round(longest_m / 1000.0, 2),
...
def merge_dedupe(existing: Optional[List[Any]], new_items: Optional[List[Any]]) -> List[Dict[str, Any]]:
    ...
    def keyfunc(a: Dict[str, Any]) -> str:
        sd = a.get("start_date_local") or a.get("start_date") or ""
        return str(sd)
...
def _validate_time_args(args: argparse.Namespace) -> None:
    ...
    if args.after:
        try:
            ...
            raise SystemExit("--after must be in YYYY-MM-DD format.")
    if args.before:
        try:
...
def main() -> None:
    args = parse_args()
    _validate_time_args(args)
    load_dotenv()

    base_url = os.getenv("STRAVA_BASE_URL", "https://www.strava.com")
    client_id = os.getenv("STRAVA_CLIENT_ID")
    client_secret = os.getenv("STRAVA_CLIENT_SECRET")
    auth_code = os.getenv("STRAVA_AUTH_CODE")  # only needed for first run
    tokens_file = os.getenv("STRAVA_TOKENS_FILE", ".tokens.json")

    if not (client_id and client_secret):
        raise SystemExit("Please set STRAVA_CLIENT_ID and STRAVA_CLIENT_SECRET in .env")
    ...
    # Ensure we have an access token (exchange or refresh as needed)
    try:
        if args.verbose:
        ...
        if ("No valid tokens found and no STRAVA_AUTH_CODE provided" in msg) or ("No valid tokens found" in msg):
            try:
                ...
                redirect_host = os.getenv("STRAVA_REDIRECT_HOST", "127.0.0.1")
                redirect_port = int(os.getenv("STRAVA_REDIRECT_PORT", "8723"))
                scope = os.getenv("STRAVA_SCOPE", "read,activity:read_all")
                open_browser = os.getenv("STRAVA_OPEN_BROWSER", "true").lower() in ("1", "true", "yes", "y")
                print("Starting local OAuth flow...")
                code = run_local_authorization_flow(
                    client_id=str(client_id),
                    scope=scope,
                ...
                client.ensure_access_token(auth_code=code)
            except TimeoutError:
                raise SystemExit(
                    "Timed out waiting for Strava authorization in the browser. "
                    "Try again or set STRAVA_AUTH_CODE in .env for a manual exchange."
                )
        else:
            raise
    ...
    if args.after_epoch is not None:
        after = args.after_epoch
    elif args.after:
        after = to_epoch_seconds_from_date_str(args.after)
    else:
        raise SystemExit("Please provide --after or --after-epoch")

    if args.before_epoch is not None:
        before = args.before_epoch
    elif args.before:
    ...
    if not activities and not user_provided_before:
        if args.verbose:
            print("No activities in after..now window. Trying the most recent day with results...")
        latest = client.get_activities(
            after=after,
            before=before,
            per_page=1,
            max_pages=10,
        ...
        if latest:
            ...
            if latest_day:
                ...
                if args.verbose:
                    print(f"Refetching up to end of {latest_day} (exclusive epoch {before})...")
                activities = client.get_activities(
                    after=after,
                    before=before,
                    per_page=args.per_page,
    ...
    if args.append:
        print(f"Merged dataset size: {len(merged)} activities -> {out_path}")
    else:
